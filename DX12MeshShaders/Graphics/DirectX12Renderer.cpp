...
{
	ThrowIfFailed(commandAllocator[bufferIndex]->Reset(), "DirectX12Renderer::FrameStart: Command Allocator resetting error!");
	ThrowIfFailed(commandList->Reset(commandAllocator[bufferIndex].Get(), nullptr), "DirectX12Renderer::FrameStart: Command List resetting error!");
...
void Graphics::DirectX12Renderer::ReleaseResources()
{
	for (uint32_t bufferId = 0; bufferId < SWAP_CHAIN_BUFFER_COUNT; bufferId++)
	{
		commandAllocator[bufferId].Reset();
